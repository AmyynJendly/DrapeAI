...
export default function TryOnModal({ product, onClose }: TryOnModalProps) {
  ...
  const handleProcessTryOn = async () => {
    ...
    try {
      ...
      clearTimeout(step1);
      clearTimeout(step2);
      console.warn('API error, fallback to client render preview:', err);
      // Fallback try-on result render preview
      setTimeout(() => {
        setResult({
          id: 'demo-' + Date.now(),
          productId: product.id,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-[28px] max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-black/10 relative my-8">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-6 right-6 p-2 rounded-full bg-[#F0EEED] hover:bg-black hover:text-white text-black transition"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="flex items-center gap-4 pb-6 border-b border-black/10">
          <img
            ...
            alt={product.name}
            className="w-14 h-14 object-cover rounded-xl bg-[#F0EEED]"
          />
          <div>
            <span className="text-[10px] font-extrabold uppercase tracking-widest bg-black text-white px-2 py-0.5 rounded-full">
              Virtual Try-On
            </span>
        ...
        {result ? (
          ...
          <div className="pt-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="aspect-square rounded-2xl overflow-hidden bg-[#F0EEED] border border-black/10">
              ...
              <div className="space-y-2">
                ...
                <div className="aspect-square rounded-2xl overflow-hidden bg-[#F0EEED] border-2 border-black shadow-lg relative group">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex-1 bg-black text-white text-xs py-3.5 rounded-full font-bold hover:bg-black/80 flex items-center justify-center gap-2 transition shadow-md"
              >
                <Download className="w-4 h-4 text-yellow-400" /> Download Result Image
              </a>
              <button
                onClick={handleReset}
                className="px-6 py-3.5 rounded-full bg-[#F0EEED] text-black font-bold text-xs hover:bg-black/10 transition flex items-center justify-center gap-1.5"
              >
          ...
          /* Processing View: 3-step animated progress */
          ...
          <div className="pt-6 space-y-6">
            ...
            <div className="flex bg-[#F0EEED] p-1 rounded-full max-w-md mx-auto">
              <button
                ...
                className={`flex-1 py-2 rounded-full font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition ${
                ...
                {selectedImage ? (
                  ...
                  <label className="border-2 border-dashed border-black/20 hover:border-black rounded-2xl p-8 flex flex-col items-center justify-center gap-3 cursor-pointer bg-[#F0EEED]/50 hover:bg-[#F0EEED] transition group">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-black/50">Supports JPEG, PNG up to 10MB</p>
                    </div>
                    <input
                      type="file"
                      accept="image/*"
            ...
            <button
              ...
              className="w-full bg-black text-white py-4 rounded-full font-bold text-sm hover:bg-black/80 flex items-center justify-center gap-2 transition-all shadow-xl disabled:opacity-40 active:scale-95"
            >
              Generate AI Try-On ✨
